...
let of_tuple_fields fields = List.map of_tuple_field fields
...
let of_args args = List.map of_arg args
...
and of_pattern_data : type a b . of_pat_desc:(a -> b) -> a OCaml.pattern_data -> b pattern_data =
  ...
  let pat_extra = pat_data.pat_extra in
...
and of_pattern_desc : type k . k OCaml.pattern_desc -> k pattern_desc = function
  ...
  | Tpat_tuple fields ->
      let fields = of_tuple_fields fields in
      Tpat_tuple { fields }
...
and of_record_label_definition :
  ...
  | Kept type_expr ->
      let mut = not_available until_500 in
      Kept { type_expr; mut }
...
and of_functor_parameter : OCaml.functor_parameter -> functor_parameter =
  function
  | Unit -> Unit
...
and of_core_type_desc : OCaml.core_type_desc -> core_type_desc = function
  ...
  | Ttyp_constr (path, longid, params) ->
      Ttyp_constr { path; longid; params }
  | Ttyp_object (fields, closed) -> Ttyp_object { fields; closed }
...
and of_constructor_arguments :
  ...
  | Cstr_record label_decls ->
      let label_decls = label_decls in
      Cstr_record { label_decls }
