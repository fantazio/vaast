...
and to_pattern_data : type a b . to_pat_desc:(a -> b) -> a pattern_data -> b OCaml.pattern_data =
  ...
  let pat_extra = pat_data.pat_extra
  in
  let pat_type = pat_data.pat_type in
  let pat_env = pat_data.pat_env in
  let pat_attributes = pat_data.pat_attributes in
...
and to_pattern_desc : type k . k pattern_desc -> k OCaml.pattern_desc = function
  ...
  | Tpat_exception { pat } -> Tpat_exception pat
...
and to_expression : expression -> OCaml.expression = fun expr ->
  ...
  let exp_extra = expr.exp_extra in
  let exp_type = expr.exp_type in
  let exp_env = expr.exp_env in
  let exp_attributes = expr.exp_attributes in
  { exp_desc; exp_loc; exp_extra; exp_type; exp_env; exp_attributes }

and to_exp_extra : exp_extra -> OCaml.exp_extra = function
  | Texp_constraint { type_ } -> Texp_constraint (type_)
  | Texp_coerce { from_type; to_type } -> Texp_coerce (from_type, to_type)
  | Texp_poly { type_ } -> Texp_poly (type_)
...
and to_expression_desc : expression_desc -> OCaml.expression_desc = function
  ...
  | Texp_object { class_strc; meths } -> Texp_object (class_strc, meths)
  | Texp_pack { mod_expr } -> Texp_pack mod_expr
  ...
  | Texp_struct_item { struct_item; in_ } ->
      ...
      begin match texp_struct_item with
        ...
        | Texp_letexception { extension_ctor } ->
            Texp_letexception (extension_ctor, in_)
        | Texp_open { open_decl } -> Texp_open (open_decl, in_)
      end
      #elif OCAML_VERSION >= (5, 5, 0)
      let struct_item =
        match struct_item with
...
and to_function_body : function_body -> OCaml.function_body = fun body ->
  ...
  *)
  ...
  assert false (* for type checking. The line above already triggers it *)
  #elif OCAML_VERSION >= (5, 2, 0)
  match body with
  | Tfunction_body { expr } -> Tfunction_body expr
...
and to_record_label_definition :
  ...
  | Kept { type_expr; mut } ->
      ...
      let mut = get_since_500 mut in
      Kept (type_expr, mut)
      #endif
  | Overridden { longid; expr } -> Overridden (longid, expr)
...
and to_class_expr : class_expr -> OCaml.class_expr = fun cl ->
  ...
  let cl_attributes = cl.cl_attributes in
...
and to_class_structure : class_structure -> OCaml.class_structure = fun cstr ->
  let cstr_self = cstr.cstr_self in
  let cstr_fields = cstr.cstr_fields in
...
and to_class_field : class_field -> OCaml.class_field = fun cf ->
  ...
  let cf_attributes = cf.cf_attributes in
  { cf_desc; cf_loc; cf_attributes }

and to_class_field_kind : class_field_kind -> OCaml.class_field_kind = function
  | Tcfk_virtual { type_ } -> Tcfk_virtual type_
  | Tcfk_concrete{ over; expr } -> Tcfk_concrete (over, expr)
...
and to_class_field_desc : class_field_desc -> OCaml.class_field_desc = function
  ...
  | Tcf_attribute { attribute } -> Tcf_attribute attribute
...
and to_module_expr : module_expr -> OCaml.module_expr = fun me ->
  ...
  let mod_attributes = me.mod_attributes in
...
and to_module_expr_desc : module_expr_desc -> OCaml.module_expr_desc = function
  ...
  | Tmod_apply_unit { ftor } ->
      #if OCAML_VERSION < (5, 1, 0)
      (* Tmod_apply_unit was introduced in OCaml 5.1 *)
...
and to_structure : structure -> OCaml.structure = fun strc ->
  let str_items = strc.str_items in
...
and to_structure_item_desc : structure_item_desc -> OCaml.structure_item_desc =
  function
  | Tstr_eval { expr; attributes } -> Tstr_eval (expr, attributes)
  | Tstr_value { rec_; bindings } -> Tstr_value (rec_, bindings)
  | Tstr_primitive { val_desc } ->
      let val_desc = to_value_description val_desc in
      Tstr_primitive val_desc
  | Tstr_type { rec_; type_decls } -> Tstr_type (rec_, type_decls)
  | Tstr_typext { type_ext } -> Tstr_typext type_ext
  | Tstr_exception { type_exc } -> Tstr_exception type_exc
  | Tstr_module { mod_binding } -> Tstr_module mod_binding
  | Tstr_recmodule { mod_bindings } -> Tstr_recmodule mod_bindings
  | Tstr_modtype { modtyp_decl } -> Tstr_modtype modtyp_decl
  | Tstr_open { open_decl } -> Tstr_open open_decl
  | Tstr_class { class_bindings } -> Tstr_class class_bindings
  | Tstr_class_type { class_types } -> Tstr_class_type class_types
  | Tstr_include { incl_decl } -> Tstr_include incl_decl
  | Tstr_attribute { attribute } -> Tstr_attribute attribute
...
and to_module_binding : module_binding -> OCaml.module_binding = fun mb ->
  ...
  let mb_attributes = mb.mb_attributes in
...
and to_module_type : module_type -> OCaml.module_type = fun mty ->
    ...
    let mty_attributes = mty.mty_attributes in
...
and to_signature : signature -> OCaml.signature = fun sign ->
  let sig_items = sign.sig_items in
...
and to_signature_item_desc : signature_item_desc -> OCaml.signature_item_desc =
  ...
  | Tsig_value { val_desc } ->
      let val_desc = to_value_description val_desc in
      Tsig_value val_desc
  | Tsig_type { rec_; type_decls } -> Tsig_type (rec_, type_decls)
  | Tsig_typesubst { type_decls } -> Tsig_typesubst type_decls
  | Tsig_typext { type_ext } -> Tsig_typext type_ext
  | Tsig_exception { type_exc } -> Tsig_exception type_exc
  | Tsig_module { mod_decl } -> Tsig_module mod_decl
  | Tsig_modsubst { mod_subst } -> Tsig_modsubst mod_subst
  | Tsig_recmodule { mod_delcs } -> Tsig_recmodule mod_delcs
  | Tsig_modtype { modtype_decl } -> Tsig_modtype modtype_decl
  | Tsig_modtypesubst { modtype_decl } -> Tsig_modtypesubst modtype_decl
  | Tsig_open { open_desc } ->
      let open_desc = to_open_description open_desc in
      Tsig_open open_desc
  ...
  | Tsig_class { class_descs } ->
      ...
      Tsig_class class_descs
  | Tsig_class_type { classtype_decls } -> Tsig_class_type classtype_decls
  | Tsig_attribute { attribute } -> Tsig_attribute attribute
...
and to_module_declaration : module_declaration -> OCaml.module_declaration =
  ...
  let md_attributes = md.md_attributes in
...
and to_module_substitution : module_substitution -> OCaml.module_substitution =
  ...
  let ms_attributes = ms.ms_attributes in
...
and to_open_infos:
  ...
  let open_attributes = oi.open_attributes in
...
and to_include_infos:
  ...
  let incl_attributes = ii.incl_attributes in
...
and to_with_constraint : with_constraint -> OCaml.with_constraint = function
  | Twith_type { type_decl } -> Twith_type type_decl
  | Twith_module { path; longid } -> Twith_module (path, longid)
  | Twith_modtype { mod_type } -> Twith_modtype mod_type
  | Twith_typesubst { type_decl } -> Twith_typesubst type_decl
  | Twith_modsubst { path; longid } -> Twith_modsubst (path, longid)
  | Twith_modtypesubst { mod_type } -> Twith_modtypesubst mod_type
...
and to_core_type : core_type -> OCaml.core_type = fun ct ->
  ...
  let ctyp_attributes = ct.ctyp_attributes in
...
and to_core_type_desc : core_type_desc -> OCaml.core_type_desc = function
  ...
  | Ttyp_variant { rows; closed; labels } ->
      Ttyp_variant (rows, closed, labels)
  | Ttyp_poly { params; type_ } -> Ttyp_poly (params, type_)
  | Ttyp_package { pack_type } -> Ttyp_package (pack_type)
  | Ttyp_open { path; longid; type_ } ->
      #if OCAML_VERSION < (5, 2, 0)
      (* Ttyp_open was introduced in OCaml 5.2 *)
  ...
  | Ttyp_functor { arg_label; arg_name; arg_type; res_type } ->
      #if OCAML_VERSION < (5, 5, 0)
      (* Ttyp_functor was introduced in OCaml 5.5 *)
      ignore arg_label; (* remove warning 27 *)
      ignore arg_name; (* remove warning 27 *)
      ignore arg_type; (* remove warning 27 *)
      ignore res_type; (* remove warning 27 *)
      assert false
      #elif OCAML_VERSION >= (5, 5, 0)
      Ttyp_functor (arg_label, arg_name, arg_type, res_type)
      #endif
...
and to_type_declaration : type_declaration -> OCaml.type_declaration = fun td ->
  ...
  #elif OCAML_VERSION >= (5, 5, 0)
  let typ_constraints = td.typ_cstrs in
  #endif
  ...
  let typ_attributes = td.typ_attributes in
  { typ_id;
    typ_name;
    typ_params;
    typ_type;
...
and to_type_kind : type_kind -> OCaml.type_kind = function
  ...
  | Ttype_external { name } ->
      #if OCAML_VERSION < (5, 5, 0)
      (* Ttype_external was introduced in OCaml 5.5 *)
      ignore name; (* remove warning 27 *)
      assert false;
      #elif OCAML_VERSION >= (5, 5, 0)
      Ttype_external name
      #endif
...
and to_constructor_arguments :
  ...
  function
  | Cstr_tuple { fields } -> Cstr_tuple fields
  | Cstr_record { label_decls } -> Cstr_record label_decls
...
and to_type_extension : type_extension -> OCaml.type_extension = fun te ->
  ...
  let tyext_params = te.tyext_params in
  let tyext_constructors = te.tyext_constructors in
  let tyext_private = te.tyext_private in
  let tyext_loc = te.tyext_loc in
  let tyext_attributes = te.tyext_attributes in
  { tyext_path;
    tyext_txt;
    tyext_params;
    tyext_constructors;
...
and to_type_exception : type_exception -> OCaml.type_exception = fun te ->
  let tyexn_constructor = te.tyexn_constructor in
  let tyexn_loc = te.tyexn_loc in
  let tyexn_attributes = te.tyexn_attributes in
...
and to_class_type : class_type -> OCaml.class_type = fun ct ->
  ...
  let cltyp_attributes = ct.cltyp_attributes in
...
and to_class_type_desc : class_type_desc -> OCaml.class_type_desc = function
  | Tcty_constr { path; longid; params } ->
      Tcty_constr (path, longid, params)
  | Tcty_signature { class_sign } -> Tcty_signature class_sign
...
and to_class_signature : class_signature -> OCaml.class_signature = fun cs ->
  let csig_self = cs.csig_self in
  let csig_fields = cs.csig_fields in
...
and to_class_type_field : class_type_field -> OCaml.class_type_field = fun cf ->
  ...
  let ctf_attributes = cf.ctf_attributes in
  { ctf_desc; ctf_loc; ctf_attributes }

and to_class_type_field_desc :
  class_type_field_desc -> OCaml.class_type_field_desc =
  function
  | Tctf_inherit { parent_type } -> Tctf_inherit parent_type
  | Tctf_val { name; mut; virt; type_ } -> Tctf_val (name, mut, virt, type_)
  | Tctf_method { name; priv; virt; type_ } ->
      Tctf_method (name, priv, virt, type_)
  | Tctf_constraint { type1; type2 } -> Tctf_constraint (type1, type2)
  | Tctf_attribute { attribute } -> Tctf_attribute attribute
...
and to_class_infos :
  ...
  let ci_attributes = ci.ci_attributes in
  { ci_virt;
    ci_params;
    ci_id_name;
    ci_id_class;
